...
## Correlation between erythema index variability and melanin index for combined face and palm data

```{r ei-sd-facepalm, fig.width=10, fig.height=6}
plot_facepalm_ei = ggplot(data = facepalm_spectrum) + geom_point(aes(mi, ei), color = "purple", size = 0.8) + theme_classic() +
  labs(title = "Forehead/Cheek and Palm",
       x = "Melanin index",
       y = "Erythema index") + xlim(0, 120)
window_size = 20 # melanin index units
step_size = 1 # melanin index units
# face and palm combined
facepalm_mi_seq = seq(floor(quantile(facepalm_spectrum$mi, probs = 0.01)), ceiling(quantile(facepalm_spectrum$mi, probs = 0.99)), by = step_size)
facepalm_ei_sd = sapply(facepalm_mi_seq, function(mi_val) {
  window_data = facepalm_spectrum %>% filter(mi >= (mi_val - window_size/2) & mi < (mi_val + window_size/2))
  sd(window_data$ei)
})
facepalm_ei_sd_df = data.frame(mi = facepalm_mi_seq, ei_sd = facepalm_ei_sd)
plot_facepalm_ei_sd = ggplot(data = facepalm_ei_sd_df) + geom_line(aes(mi, ei_sd), color = "purple") + theme_classic() +
  labs(title = "Forehead/Cheek and Palm",
       x = "Melanin index",
       y = "Erythema index standard deviation") + xlim(0, 120)

wrap_plots(plot_facepalm_ei, plot_facepalm_ei_sd, nrow = 1, guides = "collect") + plot_annotation(title = "Correlation between erythema index and melanin index") & theme(legend.position = "bottom")

pdf("output/facepalm_ei_mi_correlation.pdf", width = 10, height = 6)
wrap_plots(plot_facepalm_ei, plot_facepalm_ei_sd, nrow = 1, guides = "collect") + plot_annotation(title = "Correlation between erythema index and melanin index") & theme(legend.position = "bottom")
dev.off()
```