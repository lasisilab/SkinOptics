...
- Erythema index (EI) is calculated as EI = 100 * log10(R680 / ((R550 + R560)/2)), where R550 and R560 are the reflectance percentages at 550 nm and 560 nm, respectively.
...
face_spectrum = data.frame()
for (id in face_ids) {
  spectrum_id = spectrum %>% filter(C == id)
  if (any(spectrum_id$site == "Forehead")) {
    selected_spectrum = spectrum_id %>% filter(site == "Forehead") %>% slice_sample(n = 1)
  } else {
    selected_spectrum = spectrum_id %>% filter(site == "Cheek") %>% slice_sample(n = 1)
  }
  face_spectrum = rbind(face_spectrum, selected_spectrum)
}
# face_spectrum = spectrum %>% filter(site %in% c("Cheek", "Forehead"))
face_spectrum$site = "Forehead/Cheek"
palm_spectrum = spectrum %>% filter(C %in% face_ids, site == "Palm")
```
...
```{r plot-raw-reflectance-by-class-face, fig.width=10, fig.height=3.5}
plots_faceraw = list()
for (cls in levels(eumelanin_classes_face)) {
  cls_data = face_spectrum %>% filter(class == cls)
  p = PlotReflectance(cls_data) + labs(
        x = "Wavelength (nm)",
        y = "Reflectance",
        title = cls,
...
wrap_plots(plots_faceraw, nrow = 1, guides = "collect") + plot_annotation(title = "Face reflectance by Face eumelanin classes") & theme(legend.position = "bottom")

pdf("output/reflectance_face.pdf", width = 10, height = 3.5)
wrap_plots(plots_faceraw, nrow = 1, guides = "collect") + plot_annotation(title = "Face reflectance by Face eumelanin classes") & theme(legend.position = "bottom")
dev.off()
```
...
```{r plot-raw-reflectance-by-class-palm, fig.width=10, fig.height=4}
plots_palmraw = list()
for (cls in levels(eumelanin_classes_palm)) {
  cls_data = palm_spectrum %>% filter(palm_class == cls)
  p = PlotReflectance(cls_data) + labs(
        x = "Wavelength (nm)",
        y = "Reflectance",
        title = cls,
...
wrap_plots(plots_palmraw, nrow = 1, guides = "collect") + plot_annotation(title = "Palm reflectance by Palm eumelanin classes") & theme(legend.position = "bottom")
pdf("output/reflectance_palm.pdf", width = 10, height = 4)
wrap_plots(plots_palmraw, nrow = 1, guides = "collect") + plot_annotation(title = "Palm reflectance by Palm eumelanin classes") & theme(legend.position = "bottom")
dev.off()
```

### Palm reflectance by face eumelanin class
```{r plot-raw-reflectance-by-class-palm-face, fig.width=10, fig.height=3.5}
plots_palmraw = list()
for (cls in levels(eumelanin_classes_face)) {
  cls_data = palm_spectrum %>% filter(face_class == cls)
  p = PlotReflectance(cls_data) + labs(
        x = "Wavelength (nm)",
        y = "Reflectance",
        title = cls,
...
wrap_plots(plots_palmraw, nrow = 1, guides = "collect") + plot_annotation(title = "Palm reflectance by Face eumelanin classes") & theme(legend.position = "bottom")
pdf("output/reflectance_palm_face.pdf", width = 10, height = 3.5)
wrap_plots(plots_palmraw, nrow = 1, guides = "collect") + plot_annotation(title = "Palm reflectance by Face eumelanin classes") & theme(legend.position = "bottom")
dev.off()
```
...
wrap_plots(plots_facer, nrow = 1, guides = "collect") + plot_annotation(title = "First derivatives of Face reflectance by Face eumelanin classes") & theme(legend.position = "bottom")
...
plots_palmr = list()
for (cls in levels(eumelanin_classes_palm)) {
  cls_data = drv_palmr_df %>% filter(palm_class == cls)
  p = PlotDerivative(cls_data) + labs(
        x = "Wavelength (nm)",
        y = "Derivative",
        title = cls,
...
wrap_plots(plots_palmr, nrow = 1, guides = "collect") + plot_annotation(title = "First derivatives of Palm reflectance by Palm eumelanin classes") & theme(legend.position = "bottom")
...
```{r plot-reflectance-by-class-palm-face, fig.width=10, fig.height=3.5}
plots_palmr = list()
for (cls in levels(eumelanin_classes_face)) {
  cls_data = drv_palmr_df %>% filter(face_class == cls)
  p = PlotDerivative(cls_data) + labs(
        x = "Wavelength (nm)",
        y = "Derivative",
        title = cls,
...
wrap_plots(plots_palmr, nrow = 1, guides = "collect") + plot_annotation(title = "First derivatives of Palm reflectance by Face eumelanin classes") & theme(legend.position = "bottom")

pdf("output/reflectance_derivative_palm_face.pdf", width = 10, height = 3.5)
wrap_plots(plots_palmr, nrow = 1, guides = "collect") + plot_annotation(title = "First derivatives of Palm reflectance by Face eumelanin classes") & theme(legend.position = "bottom")
dev.off()
```
...
wrap_plots(plots_facea, nrow = 1, guides = "collect") + plot_annotation(title = "First derivatives of Face absorbance by Face eumelanin classes") & theme(legend.position = "bottom")
...
plots_palma = list()
for (cls in levels(eumelanin_classes_palm)) {
  cls_data = drv_palma_df %>% filter(palm_class == cls)
  p = PlotDerivative(cls_data) + labs(
        x = "Wavelength (nm)",
        y = "Derivative",
        title = cls,
...
wrap_plots(plots_palma, nrow = 1, guides = "collect") + plot_annotation(title = "First derivatives of Palm absorbance by Palm eumelanin classes") & theme(legend.position = "bottom")
...
```{r plot-absorbance-by-class-palm-face, fig.width=10, fig.height=3.5}
plots_palma = list()
for (cls in levels(eumelanin_classes_face)) {
  cls_data = drv_palma_df %>% filter(face_class == cls)
  p = PlotDerivative(cls_data) + labs(
        x = "Wavelength (nm)",
        y = "Derivative",
        title = cls,
...
wrap_plots(plots_palma, nrow = 1, guides = "collect") + plot_annotation(title = "First derivatives of Palm absorbance by Face eumelanin classes") & theme(legend.position = "bottom")

pdf("output/absorbance_derivative_palm_face.pdf", width = 10, height = 3.5)
wrap_plots(plots_palma, nrow = 1, guides = "collect") + plot_annotation(title = "First derivatives of Palm absorbance by Face eumelanin classes") & theme(legend.position = "bottom")
dev.off()
```
...
plot_face_ref = ggplot(data = sd_face) + geom_point(aes(log_mi, sd_ref), color = "pink", size = 0.8) + theme_classic() +
  labs(title = paste0("Forehead/Cheek Reflectance (n=", nrow(sd_face), ")"),
       x = "log(Melanin index)",
...
plot_palm_ref = ggplot(data = sd_palm) + geom_point(aes(log_mi, sd_ref), color = "steelblue", size = 0.8) + theme_classic() +
  labs(title = paste0("Palm Reflectance (n=", nrow(sd_palm), ")"),
       x = "log(Melanin index)",
       y = "standard deviation of derivatives") + ylim(0, 0.25) + xlim(2.75, 4.75)
...
plot_face_abs = ggplot(data = sd_face) + geom_point(aes(log_mi, sd_abs), color = "pink", size = 0.8) + theme_classic() +
  labs(title = paste0("Forehead/Cheek Absorbance (n=", nrow(sd_face), ")"),
       x = "log(Melanin index)",
...
plot_palm_abs = ggplot(data = sd_palm) + geom_point(aes(log_mi, sd_abs), color = "steelblue", size = 0.8) + theme_classic() +
  labs(title = paste0("Palm Absorbance (n=", nrow(sd_palm), ")"),
       x = "log(Melanin index)",
       y = "standard deviation of derivatives") + ylim(0, 0.0055) + xlim(2.75, 4.75)
...
plot_face_ei = ggplot(data = face_spectrum) + geom_point(aes(mi, ei), color = "pink", size = 0.8) + theme_classic() +
  labs(title = paste0("Forehead/Cheek (n=", nrow(face_spectrum), ")"),
       x = "Forehead/Cheek Melanin index",
...
plot_palm_ei = ggplot(data = palm_spectrum) + geom_point(aes(mi, ei), color = "steelblue", size = 0.8) + theme_classic() +
  labs(title = paste0("Palm (n=", nrow(palm_spectrum), ")"),
       x = "Palm Melanin index",
       y = "Erythema index") + xlim(0, 120) + ylim(10, 55)
...
```{r palmei-facemi-correlation, fig.width=4, fig.height=4}
palm_spectrum$face_mi = face_spectrum$mi[match(palm_spectrum$C, face_spectrum$C)]
plot_palmei_facemi = ggplot(data = palm_spectrum) + geom_point(aes(face_mi, ei), color = "purple", size = 0.8) + theme_classic() +
  labs(title = paste0("Palm EI vs Face MI (n=", nrow(palm_spectrum), ")"),
       x = "Face Melanin index",
       y = "Palm Erythema index") + xlim(0, 120) + ylim(10, 55)

print(plot_palmei_facemi)
pdf("output/palm_ei_face_mi_correlation.pdf", width = 4, height = 4)
plot_palmei_facemi
dev.off()
```
...
```{r ei-sd, fig.width=10, fig.height=6}
...
face_ei_sd_df = data.frame(mi = face_mi_seq, ei_sd = face_ei_sd)
plot_face_ei_sd = ggplot(data = face_ei_sd_df) + geom_line(aes(mi, ei_sd), color = "pink") + theme_classic() +
  labs(title = "Forehead/Cheek",
       x = "Melanin index",
       y = "Erythema index standard deviation") + xlim(0, 120)
...
palm_ei_sd_df = data.frame(mi = palm_mi_seq, ei_sd = palm_ei_sd)
max_mi = palm_ei_sd_df$mi[which.max(palm_ei_sd_df$ei_sd)]
plot_palm_ei_sd = ggplot(data = palm_ei_sd_df) + geom_line(aes(mi, ei_sd), color = "steelblue") + theme_classic() +
  labs(title = "Palm",
       x = "Melanin index",
       y = "Erythema index standard deviation") + xlim(0, 120) +
...
wrap_plots(plot_face_ei_sd, plot_palm_ei_sd, nrow = 1, guides = "collect") + plot_annotation(title = "Correlation between erythema index sd and melanin index") & theme(legend.position = "bottom")

pdf("output/ei_sd_mi_correlation.pdf", width = 10, height = 6)
wrap_plots(plot_face_ei_sd, plot_palm_ei_sd, nrow = 1, guides = "collect") + plot_annotation(title = "Correlation between erythema index sd and melanin index") & theme(legend.position = "bottom")
dev.off()
```

```{r palmei-facemi-sd, fig.width=4, fig.height=4}
...
max_mi = palm_ei_sd_df$mi[which.max(palm_ei_sd_df$ei_sd)]
plot_palm_ei_sd = ggplot(data = palm_ei_sd_df) + geom_line(aes(mi, ei_sd), color = "purple") + theme_classic() +
...
print(plot_palm_ei_sd)
pdf("output/palm_ei_face_mi_sd_correlation.pdf", width = 4, height = 4)
plot_palm_ei_sd
dev.off()
```
