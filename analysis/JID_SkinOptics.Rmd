...
annotation_df <- data.frame(
  ...
  label = c(
    sprintf("slope = %.2f\nR² = %.3f\np %s", face_slope, face_r2, 
            ifelse(face_p < 0.001, "< 0.001", sprintf("= %.3f", face_p))),
    sprintf("slope = %.2f\nR² = %.3f\np %s", palm_slope, palm_r2,
            ifelse(palm_p < 0.001, "< 0.001", sprintf("= %.3f", palm_p)))
  )
)
...
p <- ggplot(data = sd_combined, aes(x = mi, y = sd_ref, color = site)) + 
  ...
  scale_color_manual(values = c("Forehead/Cheek" = "pink", "Palm" = "steelblue")) +
  facet_wrap(~site, nrow = 1, labeller = as_labeller(sd_site_labels)) +
  geom_text(data = annotation_df, aes(x = 25, y = Inf, label = label),
            hjust = 0, vjust = 1.2, size = 3, color = "black", inherit.aes = FALSE) +
  theme_classic() +
...
p <- ggplot(peak_valley_combined %>% filter(delta > 0), 
  ...
  scale_color_manual(values = c("Forehead/Cheek" = "pink", "Palm" = "steelblue")) +
  facet_wrap(~site, nrow = 1, labeller = as_labeller(pv_site_labels)) +
  geom_text(data = annotation_delta, aes(x = 20, y = Inf, label = label),
            hjust = 0, vjust = 1.2, size = 3, color = "black", inherit.aes = FALSE) +
  theme_classic() +
...
p <- ggplot(data = sd_combined, aes(x = mi, y = sd_abs, color = site)) + 
  ...
  scale_color_manual(values = c("Forehead/Cheek" = "pink", "Palm" = "steelblue")) +
  facet_wrap(~site, nrow = 1, labeller = as_labeller(sd_site_labels)) +
  geom_text(data = annotation_abs, aes(x = 25, y = Inf, label = label),
            hjust = 0, vjust = 1.2, size = 3, color = "black", inherit.aes = FALSE) +
  theme_classic() +
