...
### Log-transformed: Derivative SD vs melanin index (reflectance)
...
# Helper function to format p-values (handles very small values)
...
# Extract regression stats for annotation
face_r2 <- summary(reg_face_log)$r.squared
face_slope <- coef(reg_face_log)[2]
face_p <- summary(reg_face_log)$coefficients[2, 4]

palm_r2 <- summary(reg_palm_log)$r.squared
palm_slope <- coef(reg_palm_log)[2]
palm_p <- summary(reg_palm_log)$coefficients[2, 4]

# Print regression summaries
cat("=== Log-log regression: log(sd_ref) ~ log(mi) ===\n\n")
cat("FACE:\n")
print(summary(reg_face_log))
cat("\n\nPALM:\n")
print(summary(reg_palm_log))

# Create annotation data with actual coordinates
annotation_df <- data.frame(
  site = c("Forehead/Cheek", "Palm"),
  label = c(
    sprintf("slope = %.2f\nR² = %.3f\n%s", face_slope, face_r2, format_pval(face_p)),
    sprintf("slope = %.2f\nR² = %.3f\n%s", palm_slope, palm_r2, format_pval(palm_p))
  ),
  x = 90,
  y = max(sd_combined$sd_ref) * 0.9
)
...
Log-transforming both axes tests whether peak and valley wavelength positions follow a power-law relationship with melanin content. Since wavelengths vary little, we expect minimal relationships.
...
# Extract regression stats for annotation
face_r2 <- summary(reg_face_delta_log)$r.squared
face_slope <- coef(reg_face_delta_log)[2]
face_p <- summary(reg_face_delta_log)$coefficients[2, 4]

palm_r2 <- summary(reg_palm_delta_log)$r.squared
palm_slope <- coef(reg_palm_delta_log)[2]
palm_p <- summary(reg_palm_delta_log)$coefficients[2, 4]

# Print regression summaries
cat("=== Log-log regression: log(delta) ~ log(mi) ===\n\n")
cat("FACE:\n")
print(summary(reg_face_delta_log))
cat("\n\nPALM:\n")
print(summary(reg_palm_delta_log))

# Create annotation data with actual coordinates
pv_delta_filtered <- peak_valley_combined %>% filter(delta > 0)
annotation_delta <- data.frame(
  site = c("Forehead/Cheek", "Palm"),
  label = c(
    sprintf("slope = %.2f\nR² = %.3f\n%s", face_slope, face_r2, format_pval(face_p)),
    sprintf("slope = %.2f\nR² = %.3f\n%s", palm_slope, palm_r2, format_pval(palm_p))
  ...
  x = 90,
  y = max(pv_delta_filtered$delta) * 0.9
)

# Plot delta vs MI (log-log)
p <- ggplot(pv_delta_filtered, 
...
### Log-transformed: Derivative SD vs melanin index (absorbance)
...
# Extract regression stats for annotation
face_r2 <- summary(reg_face_abs_log)$r.squared
face_slope <- coef(reg_face_abs_log)[2]
face_p <- summary(reg_face_abs_log)$coefficients[2, 4]

palm_r2 <- summary(reg_palm_abs_log)$r.squared
palm_slope <- coef(reg_palm_abs_log)[2]
palm_p <- summary(reg_palm_abs_log)$coefficients[2, 4]

# Print regression summaries
cat("=== Log-log regression: log(sd_abs) ~ log(mi) ===\n\n")
cat("FACE:\n")
print(summary(reg_face_abs_log))
cat("\n\nPALM:\n")
print(summary(reg_palm_abs_log))

# Create annotation data with actual coordinates
annotation_abs <- data.frame(
  site = c("Forehead/Cheek", "Palm"),
  label = c(
    sprintf("slope = %.2f\nR² = %.3f\n%s", face_slope, face_r2, format_pval(face_p)),
    sprintf("slope = %.2f\nR² = %.3f\n%s", palm_slope, palm_r2, format_pval(palm_p))
...
## Reflectance delta (680 nm - 555 nm) vs melanin index
...
ref_delta_combined <- bind_rows(
  face_spectrum %>% select(site, mi, ref_delta, R680, R555_approx),
  palm_spectrum %>% select(site, mi, ref_delta, R680, R555_approx)
)
...
# Summary statistics
cat("=== Reflectance delta (R680 - R555_approx) summary ===\n\n")
ref_delta_combined %>%
  group_by(site) %>%
  summarise(
...
### Visualization: Reflectance delta vs melanin index

```{r reflectance-delta-vs-mi, fig.width=10, fig.height=5, out.width="50%"}
# Helper function to format p-values (handles very small values)
format_pval <- function(p) {
  if (p < 2.2e-16) {
    return("p < 2.2e-16")
...
face_r2_lin <- summary(reg_face_lin)$r.squared
face_slope_lin <- coef(reg_face_lin)[2]
face_p_lin <- summary(reg_face_lin)$coefficients[2, 4]

palm_r2_lin <- summary(reg_palm_lin)$r.squared
palm_slope_lin <- coef(reg_palm_lin)[2]
palm_p_lin <- summary(reg_palm_lin)$coefficients[2, 4]

# Create annotation data
annotation_ref_lin <- data.frame(
  site = c("Forehead/Cheek", "Palm"),
  label = c(
    sprintf("slope = %.2f\nR² = %.3f\n%s", face_slope_lin, face_r2_lin, format_pval(face_p_lin)),
    sprintf("slope = %.2f\nR² = %.3f\n%s", palm_slope_lin, palm_r2_lin, format_pval(palm_p_lin))
  ),
  x = 100,
  y = max(ref_delta_combined$ref_delta) * 0.95
)
...
p <- ggplot(ref_delta_combined, aes(x = mi, y = ref_delta, color = site)) +
  ...
  geom_text(data = annotation_ref_lin, aes(x = x, y = y, label = label),
            hjust = 1, vjust = 1, size = 3, color = "black", inherit.aes = FALSE) +
  theme_classic() +
  labs(title = "Reflectance delta (R680 - R555) vs melanin index",
       x = "Melanin index",
       y = "Reflectance delta (R680 - R555)") +
  theme(legend.position = "none")
...
### Log-transformed: Reflectance delta vs melanin index
...
```{r reflectance-delta-vs-mi-log, fig.width=10, fig.height=5, out.width="50%"}
# Helper function to format p-values (handles very small values)
format_pval <- function(p) {
  if (p < 2.2e-16) {
    return("p < 2.2e-16")
...
# Extract regression stats
face_r2 <- summary(reg_face_delta)$r.squared
face_slope <- coef(reg_face_delta)[2]
face_p <- summary(reg_face_delta)$coefficients[2, 4]

palm_r2 <- summary(reg_palm_delta)$r.squared
palm_slope <- coef(reg_palm_delta)[2]
palm_p <- summary(reg_palm_delta)$coefficients[2, 4]

# Print regression summaries
cat("\n=== Log-log regression: log(ref_delta) ~ log(mi) ===\n\n")
cat("FACE:\n")
print(summary(reg_face_delta))
cat("\n\nPALM:\n")
print(summary(reg_palm_delta))
...
# Create annotation data
annotation_ref_delta <- data.frame(
  site = c("Forehead/Cheek", "Palm"),
  label = c(
    sprintf("slope = %.2f\nR² = %.3f\n%s", face_slope, face_r2, format_pval(face_p)),
    sprintf("slope = %.2f\nR² = %.3f\n%s", palm_slope, palm_r2, format_pval(palm_p))
  ...
  x = 90,
  y = max(ref_delta_filtered$ref_delta) * 0.9
)

p <- ggplot(ref_delta_filtered, aes(x = mi, y = ref_delta, color = site)) +
  geom_point(size = 0.8, alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE) +
  scale_x_log10(breaks = c(20, 30, 50, 100)) +
  scale_y_log10() +
  ...
  geom_text(data = annotation_ref_delta, aes(x = x, y = y, label = label),
            hjust = 1, vjust = 1, size = 3, color = "black", inherit.aes = FALSE) +
  theme_classic() +
  labs(title = "Reflectance delta (R680 - R555) vs melanin index (log-log)",
       x = "log(Melanin Index)",
       y = "log(Reflectance delta)") +
  theme(legend.position = "none")

print(p)
save_pdf(p, "reflectance_delta_vs_mi_log.pdf", width = 10, height = 5)
```
...
### Face erythema index vs face melanin index

```{r faceei-facemi-correlation, fig.width=5, fig.height=5, out.width="50%"}
...
# Extract regression stats
face_ei_r2_lin <- summary(reg_face_ei_lin)$r.squared
face_ei_slope_lin <- coef(reg_face_ei_lin)[2]
face_ei_p_lin <- summary(reg_face_ei_lin)$coefficients[2, 4]

# Print regression summary
cat("=== Linear regression: face_ei ~ face_mi ===\n")
print(summary(reg_face_ei_lin))

# Create annotation
annotation_face_ei_lin <- sprintf("slope = %.3f\nR² = %.3f\n%s", face_ei_slope_lin, face_ei_r2_lin, format_pval(face_ei_p_lin))

p <- ggplot(data = face_spectrum, aes(x = mi, y = ei)) + 
  ...
  theme_classic() +
  labs(title = paste0("Face EI vs Face MI (n=", nrow(face_spectrum), ")"),
       x = "Face Melanin index",
       y = "Face Erythema index") + 
  xlim(0, 120) + ylim(10, 55)

print(p)
save_pdf(p, "face_ei_face_mi_correlation.pdf", width = 5, height = 5)
```

### Log-transformed: Face erythema index vs face melanin index

Log-log regression tests whether face erythema index follows a power-law relationship with face melanin content.

```{r faceei-facemi-log, fig.width=5, fig.height=5, out.width="50%"}
# Helper function to format p-values (handles very small values)
format_pval <- function(p) {
  if (p < 2.2e-16) {
    return("p < 2.2e-16")
...
reg_face_ei <- lm(log(ei) ~ log(mi), data = face_spectrum %>% filter(ei > 0, mi > 0))

# Extract regression stats
face_ei_r2 <- summary(reg_face_ei)$r.squared
face_ei_slope <- coef(reg_face_ei)[2]
face_ei_p <- summary(reg_face_ei)$coefficients[2, 4]

# Print regression summary
cat("=== Log-log regression: log(face_ei) ~ log(face_mi) ===\n")
print(summary(reg_face_ei))

# Create annotation
annotation_face_ei <- sprintf("slope = %.2f\nR² = %.3f\n%s", face_ei_slope, face_ei_r2, format_pval(face_ei_p))

p <- ggplot(data = face_spectrum %>% filter(ei > 0, mi > 0), aes(x = mi, y = ei)) + 
  geom_point(color = "pink", size = 0.8, alpha = 0.7) + 
  geom_smooth(method = "lm", se = TRUE, color = "darkred") +
  scale_x_log10(breaks = c(20, 30, 50, 100)) +
  scale_y_log10() +
  annotate("text", x = 90, y = max(face_spectrum$ei) * 0.9, label = annotation_face_ei,
           hjust = 1, vjust = 1, size = 3, color = "black") +
  theme_classic() +
  labs(title = paste0("Face EI vs Face MI (log-log, n=", nrow(face_spectrum), ")"),
       x = "log(Face Melanin index)",
       y = "log(Face Erythema index)")

print(p)
save_pdf(p, "face_ei_face_mi_log.pdf", width = 5, height = 5)
```

### Palm erythema index vs face melanin index

```{r palmei-facemi-correlation, fig.width=5, fig.height=5, out.width="50%"}
...
# Extract regression stats
palm_ei_r2_lin <- summary(reg_palm_ei_lin)$r.squared
palm_ei_slope_lin <- coef(reg_palm_ei_lin)[2]
palm_ei_p_lin <- summary(reg_palm_ei_lin)$coefficients[2, 4]

# Print regression summary
cat("=== Linear regression: palm_ei ~ face_mi ===\n")
print(summary(reg_palm_ei_lin))

# Create annotation
annotation_palm_ei_lin <- sprintf("slope = %.3f\nR² = %.3f\n%s", palm_ei_slope_lin, palm_ei_r2_lin, format_pval(palm_ei_p_lin))

p <- ggplot(data = palm_spectrum, aes(x = face_mi, y = ei)) + 
...
print(p)
save_pdf(p, "palm_ei_face_mi_correlation.pdf", width = 5, height = 5)
```

### Log-transformed: Palm erythema index vs face melanin index

Log-log regression tests whether palm erythema index follows a power-law relationship with face melanin content. This cross-site comparison may reveal systemic hemoglobin patterns independent of local melanin.

```{r palmei-facemi-log, fig.width=5, fig.height=5, out.width="50%"}
# Helper function to format p-values
format_pval <- function(p) {
  if (p < 2.2e-16) {
    return("p < 2.2e-16")
...
# Run log-log regression
reg_palm_ei <- lm(log(ei) ~ log(face_mi), data = palm_spectrum %>% filter(ei > 0, face_mi > 0))

# Extract regression stats
palm_ei_r2 <- summary(reg_palm_ei)$r.squared
palm_ei_slope <- coef(reg_palm_ei)[2]
palm_ei_p <- summary(reg_palm_ei)$coefficients[2, 4]

# Print regression summary
cat("=== Log-log regression: log(palm_ei) ~ log(face_mi) ===\n")
print(summary(reg_palm_ei))

# Create annotation
annotation_palm_ei <- sprintf("slope = %.2f\nR² = %.3f\n%s", palm_ei_slope, palm_ei_r2, format_pval(palm_ei_p))

p <- ggplot(data = palm_spectrum %>% filter(ei > 0, face_mi > 0), aes(x = face_mi, y = ei)) + 
  geom_point(color = "steelblue", size = 0.8, alpha = 0.7) + 
  geom_smooth(method = "lm", se = TRUE, color = "darkblue") +
  scale_x_log10(breaks = c(20, 30, 50, 100)) +
  scale_y_log10() +
  annotate("text", x = 90, y = max(palm_spectrum$ei) * 0.9, label = annotation_palm_ei,
           hjust = 1, vjust = 1, size = 3, color = "black") +
  theme_classic() +
  labs(title = paste0("Palm EI vs Face MI (log-log, n=", nrow(palm_spectrum), ")"),
       x = "log(Face Melanin index)",
       y = "log(Palm Erythema index)")

print(p)
save_pdf(p, "palm_ei_face_mi_log.pdf", width = 5, height = 5)
```
