...
## Paired face-palm melanin index comparison

As a reminder, face eumelanin classes are defined as: Low (MI < 25), Intermediate low (25 $\leq$ MI < 50), Intermediate (50 $\leq$ MI < 75), Intermediate high (75 $\leq$ MI < 100), and High (MI $\geq$ 100).

```{r paired-face-palm-mi, fig.width=8, fig.height=5}
# Match face MI to palm data
paired_mi <- palm_spectrum %>%
  select(C, palm_mi = mi) %>%
  inner_join(face_spectrum %>% select(C, face_mi = mi, face_class = class), by = "C")

cat("Paired face-palm observations:", nrow(paired_mi), "\n\n")

# Summary table: palm MI statistics by face eumelanin class
paired_summary <- paired_mi %>%
  group_by(face_class) %>%
  summarise(
    n = n(),
    face_mi_mean = mean(face_mi),
    face_mi_sd = sd(face_mi),
    palm_mi_mean = mean(palm_mi),
    palm_mi_sd = sd(palm_mi),
    palm_mi_min = min(palm_mi),
    palm_mi_max = max(palm_mi),
    mi_diff_mean = mean(face_mi - palm_mi),
    mi_diff_sd = sd(face_mi - palm_mi),
    .groups = "drop"
  )

kable(paired_summary, digits = 1,
      col.names = c("Face eumelanin class", "n",
                     "Face MI mean", "Face MI SD",
                     "Palm MI mean", "Palm MI SD",
                     "Palm MI min", "Palm MI max",
                     "Face-Palm diff mean", "Face-Palm diff SD"),
      caption = "Palm melanin index summary by face eumelanin class")
```

```{r paired-face-palm-mi-plot, fig.width=6, fig.height=5, out.width="70%"}
# Scatter plot of face MI vs palm MI (stored for composite figure later)
mi_range <- range(c(paired_mi$face_mi, paired_mi$palm_mi))

# Run semi-log regression: palm_mi ~ log(face_mi)
reg_face_palm <- lm(palm_mi ~ log(face_mi), data = paired_mi)
fp_stats <- get_reg_stats(reg_face_palm)
print_reg_summary(reg_face_palm, "Semi-log regression: palm_mi ~ log(face_mi)")
fp_annotation <- make_annotation_label(fp_stats)

p_face_palm <- ggplot(paired_mi, aes(x = face_mi, y = palm_mi, color = face_class)) +
  geom_point(alpha = 0.5, size = 1) +
  geom_smooth(method = "lm", formula = y ~ log(x), se = TRUE, color = "black") +
  scale_x_log10() +
  ylim(mi_range) +
  annotate("text", x = max(mi_range) * 0.95, y = max(mi_range) * 0.9,
           label = fp_annotation, hjust = 1, vjust = 1, size = 3) +
  labs(x = "log(Face melanin index)", y = "Palm melanin index",
       color = "Face eumelanin class",
       title = "Paired face vs. palm melanin index (semi-log)") +
  theme_classic() +
  theme(legend.position = "none")

print(p_face_palm)
save_pdf(p_face_palm, "paired_face_palm_mi.pdf", width = 6, height = 5)
```

```{r paired-face-palm-mi-boxplot, fig.width=8, fig.height=5, out.width="70%"}
# Box plot of palm MI distribution by face eumelanin class
palm_class_n <- paired_mi %>%
  group_by(face_class) %>%
  summarise(n = n()) %>%
  mutate(label = paste0(face_class, "\n(n=", n, ")"))
palm_class_labels <- setNames(palm_class_n$label, palm_class_n$face_class)

p_boxplot <- ggplot(paired_mi, aes(x = face_class, y = palm_mi, fill = face_class)) +
  geom_boxplot(alpha = 0.7, outlier.alpha = 0.3) +
  scale_x_discrete(labels = palm_class_labels) +
  labs(x = "Face eumelanin class", y = "Palm melanin index",
       title = "Palm MI by face eumelanin class") +
  theme_classic() +
  theme(legend.position = "none")

print(p_boxplot)
save_pdf(p_boxplot, "palm_mi_by_face_class.pdf", width = 8, height = 5)
```
...
```{r composite-reflectance-derivatives, fig.width=12, fig.height=14}
...
save_pdf(composite_plot, "composite_reflectance_derivatives.pdf", width = 12, height = 14)
```
...
```{r reflectance-delta-vs-mi-log, fig.width=10, fig.height=10}
...
# Top row: scatter (a) + boxplot (b) side by side, aligned
# Boxplot first (a) so colors/classes are introduced, then scatter (b) without legend
p_top <- ggarrange(p_boxplot, p_face_palm, ncol = 2, nrow = 1,
                    labels = c("a", "b"), align = "h")
# Stack: top row, then c and d vertically
p_composite <- ggarrange(p_top, p1, p2, ncol = 1, nrow = 3,
                          labels = c("", "c", "d"))
print(p_composite)
save_pdf(p_composite, "delta_comparison_log.pdf", width = 11, height = 12)
```
...
report_lrt <- function(m0, m1, outcome_label) {
  ...
  df   <- lrt$`Df`[2]
  p    <- lrt$`Pr(>Chisq)`[2]
  cat(sprintf("%s: LRT comparing interaction vs main-effects model: χ²(%d) = %.2f, %s\n\n",
...
report_lrt(m0_pv, m1_pv, "Peak–valley derivative Δ")
...
# Report likelihood ratio test
report_lrt(m0_ref, m1_ref, "Reflectance Δ (R680–R555)")
```
...
# Peak–valley delta predicted lines
pred_pv <- ggpredict(m1_pv, terms = c("log_mi [all]", "bodysite"))
p_pv <- plot(pred_pv) +
  labs(title = "Predicted log(Δ) vs log(MI) by anatomical site",
       x = "log(Melanin Index)", y = "Predicted log(Peak–valley Δ)")
print(p_pv)
...
p_ref <- plot(pred_ref) +
  labs(title = "Predicted log(Δ) vs log(MI) by anatomical site",
       x = "log(Melanin Index)", y = "Predicted log(R680–R555 Δ)")
print(p_ref)
